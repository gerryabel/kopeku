...
<div class="p-6 bg-white rounded-xl shadow-md max-w-lg mx-auto mt-4">
    <h1 class="text-2xl font-semibold text-[#578E7E] mb-6">Tambah Foto Galeri</h1>

    <form action="{{ route('admin.photos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#578E7E]">{{ old('description') }}</textarea>
            @error('description')
            <p class="text-red-600 mt-1 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex justify-between items-center">
            ...
            <button type="submit" class="bg-[#578E7E] text-white px-6 py-2 rounded hover:bg-[#3D5B54] transition">Simpan</button>
        </div>
    </form>
</div>
