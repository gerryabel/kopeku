...
<div class="max-w-2xl mx-auto bg-white border border-gray-200 rounded-xl shadow-md p-8">
  ...
  <form action="{{ route('admin.forum.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    ...
    <div>
      ...
      {{-- Preview gambar (aktif dengan JS di bawah) --}}
      <div id="preview-images" class="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4 hidden"></div>
    </div>

    {{-- Tombol aksi --}}
    <div class="flex justify-between items-center pt-2">
      <a href="{{ route('admin.forum.index') }}" class="text-sm text-gray-600 hover:underline">
        ← Kembali
      </a>
      <button
        type="submit"
